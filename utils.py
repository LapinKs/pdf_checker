    ...
    for page in doc:
        ...
        for block in data.get("blocks", []):
            ...
            for par in paragraphs:
                ...
                if left_issue:
                    # рисуем полупрозрачную заливку слева абзаца
                    r = rect_for_lines(par)
                    # сдвигаем прямоугольник немного влево, чтобы видно было
                    left_rect = fitz.Rect(r.x0 - 30, r.y0, r.x0 + 2, r.y1)
                    page.draw_rect(left_rect, fill=LIGHT_FILL_LEFT, overlay=True)
                    page.insert_text((r.x0 - 28, r.y0), f"Неверный левый отступ (~{LEFT_MARGIN_CM} см)", fontsize=8, fontname=CYR_FONTNAME, color=(0.6,0,0))

                # правый отступ — светлая заливка справа абзаца
                if right_issue:
                    r = rect_for_lines(par)
                    right_rect = fitz.Rect(r.x1 - 2, r.y0, r.x1 + 30, r.y1)
                    page.draw_rect(right_rect, fill=LIGHT_FILL_RIGHT, overlay=True)
                    page.insert_text((r.x1 - 180, r.y0), f"Неверный правый отступ (~{RIGHT_MARGIN_CM} см)", fontsize=8, fontname=CYR_FONTNAME, color=(0,0,0.6))
